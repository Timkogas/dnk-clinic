...
export const testSlice = createSlice({
  ...
  reducers: {
    ...
    getResult: (state) => {
      ...
      const age = a[0]
      if (age < 18) {
        state.result = {name: 'молодой', description: 'Привет! Твой архетип здоровья еще формируется, и многое зависит только от тебя! Береги свое здоровье с юных лет! Побольше занимайся спортом, двигайся, будь активным, не пренебрегай полезной едой! И мы с удовольствием ждем тебя и твою семью на Чек Ап здоровья! Попроси своих родителей записать вас на прием – отправь маме ссылку.', recommendations: [], secret: '', img: '', problems: ''}
        return
      }
      if (sex === 1) { // женский
        if (age >= 18 && age <= 25) {
